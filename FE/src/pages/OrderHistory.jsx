...
export default function OrderHistory() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      {orders.length === 0 ? (
        <div className="p-6 bg-white border border-sky-100">Bạn chưa có đơn hàng</div>
      ) : (
        <div className="space-y-4">
          {orders.map(o => (
            <div key={o._id} className="p-4 bg-white border border-sky-100 rounded-md flex justify-between items-center">
              <div>
              ...
              <div className="flex items-center gap-3">
                <Link to={`/orders/${o._id}`} className="px-4 py-2 border border-sky-900 text-sky-900 rounded hover:bg-sky-900 hover:text-white transition">Xem chi tiết</Link>
              </div>
            </div>
          ))}
        </div>
